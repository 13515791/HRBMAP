...
- (void) addTemplatesForLayersInMap:(AGSMapView*)mapView
{
    ...
    //load without nib
    if (!_featureTemplatesTableView)
    {
        _featureTemplatesTableView = [[UITableView alloc]initWithFrame:self.view.bounds style:UITableViewStylePlain];
        _featureTemplatesTableView.delegate = self;
        _featureTemplatesTableView.dataSource = self;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSBundle *bundleFile = [NSBundle bundleWithPath:[[NSBundle mainBundle] pathForResource:@"EditWidget" ofType:@"bundle"]];
    UIImage *img = nil;
    if ([info.source canCreate]) {
        img = [UIImage imageWithContentsOfFile:[bundleFile pathForResource:[@"unlock" stringByDeletingPathExtension] ofType:@"png"]];
    }else{
        img = [UIImage imageWithContentsOfFile:[bundleFile pathForResource:[@"lock" stringByDeletingPathExtension] ofType:@"png"]];
    }
